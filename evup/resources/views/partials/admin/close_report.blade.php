...
<div id="staticModal-r{{$report['report'] -> reportid}}{{$report['event'] -> eventid}}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 p-4 w-full md:inset-0 h-modal md:h-full">
    <div class="relative w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="p-6 space-y-6">
                <h3 id="confirmdelTxt-{{$report['report'] -> reportid}}{{$report['event'] -> eventid}}">Would you like to delete this event?</h3>
            </div>
            <!-- Modal footer -->
            <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                <button id="confirmdelBtn-{{$report['report'] -> reportid}}{{$report['event'] -> eventid}}" onClick="delEvent({{$report['report'] -> reportid}}, {{$report['event'] -> eventid}})" data-modal-toggle="staticModal-r{{$report['report'] -> reportid}}{{$report['event'] -> eventid}}" type="button" class="del-button text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Yes</button>
