...
<article id="comment{{ $comment->commentid }}" class="p-6 mb-6 text-base bg-white rounded-lg dark:bg-gray-900" data-id="{{ $comment->commentid }}">
    <div class="flex justify-between items-center mb-2">
        <div class="flex items-center">
            <p class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white"><img class="mr-2 w-6 h-6 rounded-full" src="{{ asset('storage/images/image-'.$comment->author()->first()->userphoto.'.png')}}">
            ...
            <div class="flex gap-4">
                ...
                <form method="post" action="{{ route('edit_comment',[$comment->eventid,$comment->commentid]) }}">
                    @csrf
                    <button type="submit" title="Edit this comment" class="block text-white bg-gray-900 hover:bg-indigo-600 transition ease-in-out duration-300 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-1.5 py-1.5 text-center m-1 dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">
    ...
    <div class="w-full h-64 mt-4 bg-top bg-cover rounded-t flex flex-col justify-between" style="background-image: url({{ asset('storage/images/image-'.$comment->fileid.'.png')}})"> </div>
    @endif
    <div class="w-full md:w-full mb-2 mt-2">

        <input id="replyTextArea-{{ $comment->commentid }}" class="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full h-20 py-2 px-3 font-medium placeholder-gray-500 focus:outline-none focus:bg-white" id="commentcontent" type="text" name="commentcontent" placeholder="Type Your Reply" required>
        <div class="w-full md:w-full flex items-start md:w-full ">

            <button onclick="createNewReply(select('#comment{{$comment->commentid}}'), {{$comment->eventid}}, {{$comment->commentid}})" title="Reply to this comment" class="flex items-center mt-4 text-white bg-gray-900 hover:bg-indigo-600 transition ease-in-out duration-300 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-1.5 py-1.5 text-center  dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800" type="button">
                <svg aria-hidden="true" class="mr-1 w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
